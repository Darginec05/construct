---
title: Architecture
description: "Design goals, package layers, and the lifecycle of a flow."
---

## Design goals

1. **One stable contract.** A flow is data — a versioned, validated document
   (`@construct/dsl`). The editor, the engine, and the (cloud) copilot all target
   the same schema, so none of them is privileged.
2. **Open graph, typed built-ins.** Structurally the graph is open: a node `type`
   is any string and `config` is an opaque record, so plugins register their own
   node types. The built-in catalog ships typed config schemas and declared
   output handles, which `validateFlow` and the editor lean on.
3. **Control flow is intrinsic; leaves are pluggable.** Branching, loops,
   ...
   Everything that touches the outside world (models, tools, retrieval) is a leaf
...
```
contract      @construct/dsl        Flow / Node / Edge / Channel / Resource + validateFlow
...
leaves        @construct/nodes      agent / classifier / tool / retrieve executors
              ...
              @construct/tools      tool contract + registry (+ MCP / integrations)
...
host          @construct/server     run the engine behind an API (+ persistence)
              @construct/client     client for that API
authoring     @construct/sdk        fluent, code-first flow authoring
              apps/editor           visual flow editor (React)
```
...
1. **Author** — the editor (or `@construct/sdk`) builds a graph and serializes it
   to a DSL `Flow`.
...
copilot that generates/edits flows, and multi-tenant managed hosting. That is why
the editor ships a disconnected **Copilot** shell: it is honest that AI edits run
elsewhere and does not fake them. Everything else in the editor (canvas,
