---
title: "@construct/nodes & providers"
description: "The model/tool-backed leaf executors and the provider-neutral chat interface."
---

The control-flow runtime lives in the engine; the leaf executors that reach
external systems live in `@construct/nodes`, so the engine stays
dependency-light. Models are abstracted behind `@construct/providers`.

## `@construct/nodes` — leaf executors

Registering the package wires these executors into the engine registry:

| Executor | What it does |
|----------|--------------|
| `agent` | A model call running a bounded **tool-use loop**. Each turn the model either answers (text → loop ends) or requests tool calls, executed via `@construct/tools` and fed back as `tool` messages. Bounded by `maxSteps`; `toolChoice: "none"` collapses it to a single completion. |
...
`ToolSpec` / `ToolCall` are the wire shapes between the model and the host: a
`ToolSpec` advertises a callable (`name`, `description`, JSON-Schema
