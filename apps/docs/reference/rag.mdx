---
title: "@construct/rag"
description: "Chunking and a pluggable vector-store contract behind the retrieve node."
---
...
editor has no store registry, so it takes the id as free text.)
...
A fixed-size sliding-window splitter. Real ingestion (loaders, embedding models,
metadata extraction) is planned; today `createMemoryStore` is intended for tests
and local experimentation, not production recall quality.
