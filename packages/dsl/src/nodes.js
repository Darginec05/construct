import { z } from "zod";
import { BudgetSchema, ExprSchema, DataTypeSchema, ModelRefSchema, ToolTierSchema, } from "./primitives.js";
/**
...
const REGISTRY = new Map(BUILTIN_SPECS.map((spec) => [spec.type, spec]));
/** Register a plugin node type (or override a built-in). */
export function registerNodeSpec(spec) {
    REGISTRY.set(spec.type, spec);
}
export function getNodeSpec(type) {
 ...
 */
export function resolveNodeOutputs(type, config) {
    ...
    if (type === "router" && Array.isArray(cfg.classes)) {
        const handles = cfg.classes.map((c) => c.name);
        if (cfg.fallback)
            handles.push("fallback");
        return handles;
    }
    if (type === "switch" && Array.isArray(cfg.cases)) {
        return [...cfg.cases, "default"];
    }
    if (type === "human") {
