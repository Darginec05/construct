...
function collectRefs(value, into) {
    if (typeof value === "string") {
        for (const ref of expressionRefs(value))
            into.add(ref);
    }
...
export function validateFlow(flow, opts = {}) {
    ...
    for (const name of opts.scopeVariables ?? [])
        knownVars.add(name);
    ...
    for (const node of flow.nodes) {
        ...
        if (node.type === "tool") {
            const resource = node.config.resource;
            ...
            if (source) {
                ...
                if (handles.length > 0 && !handles.includes(edge.sourceHandle)) {
                    issues.push({
                        level: "error",
                        edgeId: edge.id,
                        message: `source handle "${edge.sourceHandle}" not in [${handles.join(", ")}] for node "${edge.source}"`,
                    });
                }
            }
        }
...
export function assertValidFlow(flow, opts = {}) {
    const errors = validateFlow(flow, opts).filter((i) => i.level === "error");
    if (errors.length > 0) {
        const lines = errors.map((e) => `  - ${e.nodeId ?? e.edgeId ?? "flow"}: ${e.message}`);
