...
const fixSuggestions = defineFlow("fix_suggestions", "Apply review fixes", (f) => {
  ...
  f.input({ schema: { findings, workspace } })
    .agent({
      model: anthropic("claude-sonnet-4-6"),
...
export const codeReview = defineFlow("code-review", "Multi-agent code review", (f) => {
  ...
  const planned = f
    ...
    .tool(runTests, { args: { suite: "unit" }, resource: ci, writeTo: workspace });
  ...
  const passGate = synthesize.branch({ condition: findings.path("pass") });
